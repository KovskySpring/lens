 ...
 * for when the full {@linkcode Lens} object is not available.
 *
...
export function lens<S, T>(recipe: LensRecipe<S, T>): Lens<S, T> {
  return {
    ...
    set: (valueOrReducer) => {
      const currentSource = recipe.source.get();

      if (typeof valueOrReducer !== "function") {
        const value = valueOrReducer as T;
        const newSource = recipe.backward(currentSource, value);
        recipe.source.set(newSource);
        return;
      }
      ...
      const newSource = recipe.backward(currentSource, newDerived);
      recipe.source.set(newSource);
    },
  };
}
