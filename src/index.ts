 ...
 * ## Usage
 *
 * A {@linkcode Lens} is created with {@linkcode lens}. It takes a {@linkcode LensRecipe}
 * to define how to read the focused value out of the source ({@linkcode LensRecipe.forward})
 * and how to write an updated value back into it ({@linkcode LensRecipe.backward}).
 *
 * - {@linkcode Lens.get} takes the source and returns the focused value.
 * - {@linkcode Lens.set} takes the new value (or a reducer that derives it from the current
 *   one) and returns a reducer `(source) => source` that applies the change to the source.
 *
 * **Pitfalls**: The focused value must not be a function. {@linkcode Lens.set}
 * assumes every function passed to it is a reducer function that takes the current
 * value and returns a new value. If the focused value is a function, it will be called
 ...
 * ### Composition
 *
 * Joins two lens with {@linkcode compose}. Use {@linkcode extend} to compose with a
 * {@linkcode LensRecipe} instead of a {@linkcode Lens}.
 *
 * ```ts
 ...
 * [@tinymirror](https://jsr.io/@tinymirror), is meant to be a small, minimal package.
 *
 * ### Memoization
 *
 * {@linkcode Lens} does no caching. {@linkcode LensRecipe.forward} runs on every
 * {@linkcode Lens.get}, and it also runs when the reducer returned by
 * {@linkcode Lens.set} is applied with a reducer argument, since the current
 * value is needed to compute the next one. Setting a plain value skips that read
 * and only calls {@linkcode LensRecipe.backward}.
 * This keeps the package simple and unopinionated.
 * You are expected to cache the getter if it is expensive (e.g. making calculation to
 * derive a value from a large data structure). If you are just accessing a field/property/index,
 * it is fine to leave as is. This type of caching is often referred to as memoization.
 *
 * ### Very deeply composed lens
 *
 * {@linkcode Lens} uses {@linkcode LensRecipe.forward} and {@linkcode LensRecipe.backward} to
 * derive a value from a source data structure. Composed {@linkcode Lens} have no idea how the
 * other {@linkcode Lens} is structured so no flattening of the access path can be done.
 * More feature-complete packages create lens by defining property paths and indexing (e.g. using
 * `.k(["user", "info", "name"])` to access `user.info.name`) allowing them to resolve the access
 * path of composed lens at runtime and access the values directly or in a loop.
 *
 * In [@tinymirror/lens](https://jsr.io/@tinymirror/lens), access is facilitated by calling functions,
 * making it stack-sensitive. Every level added by {@linkcode compose} or {@linkcode extend} is
 * another pair of {@linkcode LensRecipe.forward}/{@linkcode LensRecipe.backward} calls, and every
 * write rebuilds the source one level at a time.
 * Deeply nested lens can use a lot of the stack and impact memory usage.
 * Though possible for very deeply composed/nested lens, it's not likely you'd run into a stack-overflow.
 * Overall, It is recommended that you avoid composing lens too many levels deep. Instead, consider
 * creating a new, singular lens that directly accesses the value you want to focus on.
 *
 * ## Documentation
 *
